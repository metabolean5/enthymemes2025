...
# Prin
    ...
    #df = df[df['gold'].isin(specified_tropes)]
    ...
    return df, rst_features, trope_features, trope_columns

def train_model(X_train_text, X_train_rst, X_train_tropes, y_train, X_val_text, X_val_rst, X_val_tropes, y_val, val_ids, fold):
    ...
    # Trope features are numerical; apply weighting factor to emphasize importance
    trope_weight = 2.0  # Weighting factor to make tropes more important
    X_train_tropes = np.array(X_train_tropes) * trope_weight
    X_val_tropes = np.array(X_val_tropes) * trope_weight
    
    # Combine TF-IDF, RST, and weighted trope features
    X_train_combined = sparse.hstack([X_train_tfidf, X_train_rst, X_train_tropes])
    X_val_combined = sparse.hstack([X_val_tfidf, X_val_rst, X_val_tropes])
...
def cross_validate_model(df, rst_features, trope_features, trope_columns, n_splits=5):
    ...
    # Determine trope names for each ID
    trope_names = []
    for i in range(len(df)):
        trope_vector = trope_features[i]
        # Find indices where trope value is non-zero
        present_tropes = [trope_columns[j] for j in range(len(trope_columns)) if trope_vector[j] != 0]
        # Select the first trope alphabetically, or 'none' if no tropes are present
        trope_name = min(present_tropes) if present_tropes else 'none'
        trope_names.append(trope_name)
        ...
        for train_idx, val_idx in kf.split(texts):
            ...
            if rs == random_states[0]:
                ...
                for i in range(min(3, len(X_train_text))):
                    ...
                    print(f"RST Features (first 10 elements): {X_train_rst[i][:10]}")
                    print(f"Trope Features (first 5 elements, weighted): {X_train_tropes[i][:5]}")
                    print(f"Label: {y_train[i]} ({label_encoder.inverse_transform([y_train[i]])[0]})")

            accuracy, true_labels, preds, val_ids = train_model(
                X_train_text, X_train_rst, X_train_tropes, y_train,
                X_val_text, X_val_rst, X_val_tropes, y_val, val_ids, fold
            )
    ...
    # Create DataFrame for most predicted labels and trope names
    results_df = pd.DataFrame({
        ...
        'trope_name': trope_names
    })
    results_df.to_csv('prediction_results_text_rst_tropes.csv', index=False)
    print("\nSaved prediction results to 'prediction_results_text_rst_tropes.csv'")
...
def main():
    ...
    df, rst_features, trope_features, trope_columns = load_data(file_path)
    ...
    cross_validate_model(df, rst_features, trope_features, trope_columns)
