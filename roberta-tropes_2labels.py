...
def load_data(file_path):
    ...
    # Identify trope columns (assuming they start with 'Trope_' or are binary indicators)
    ...
    if not trope_columns:
        ...
        # Extract trope features as a matrix
    ...
    return df, trope_features

class TextTropeDataset(Dataset):
    def __init__(self, texts, labels, ids, trope_features, tokenizer, max_len=128):
        self.texts = texts
        self.labels = labels
        self.ids = ids
...
class RobertaTropeClassifier(nn.Module):
    def __init__(self, roberta_model, trope_dim, num_labels=2, dropout=0.1):
        super(RobertaTropeClassifier, self).__init__()
        self.roberta = roberta_model
        self.dropout = nn.Dropout(dropout)
        self.classifier = nn.Linear(768 + trope_dim, num_labels)

    def forward(self, input_ids, attention_mask, trope_features):
        outputs = self.roberta(input_ids=input_ids, attention_mask=attention_mask)
        cls_output = outputs.pooler_output
        combined_features = torch.cat((cls_output, trope_features), dim=1)
...
def train_model(model, train_loader, val_loader, device, fold, epochs=4, lr=2e-5, patience=4):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            outputs = model(input_ids, attention_mask, trope_features)
...
def cross_validate_model(df, trope_features, n_splits=5):
    ...
    for rs in random_states:
        ...
        for train_idx, val_idx in kf.split(texts):
            ...
            # Create datasets
            train_dataset = TextTropeDataset(X_train, y_train, train_ids, X_train_tropes, tokenizer)
            val_dataset = TextTropeDataset(X_val, y_val, val_ids, X_val_tropes, tokenizer)
            ...
            model = RobertaTropeClassifier(roberta_model, trope_dim, num_labels=2)
    ...
    results_df.to_csv('prediction_results_roberta_tropes.csv', index=False)
    print("\nSaved prediction results to 'prediction_results_roberta_tropes.csv'")
...
def main():
    ...
    df, trope_features = load_data(file_path)
